...
def simple_detect(event):
    ...
    return Decision(
        ...
        pid=pid, process_name=proc, destination_ip=dest_ip,
        is_system_process=is_sys, evidence_count=evidence,
...
def run_demo(scenario_path, out_log, use_color=True):
    ...
    lines = [l for l in scenario_path.read_text().splitlines() if l.strip()]
    total = len(lines)
    incidents = 0
    blocked = 0
    ...
    with out_log.open("a", encoding="utf-8") as logf:
        for i, raw in enumerate(lines, 1):
            ...
            elif conf >= 0.75: severity = "HIGH"
            elif conf >= 0.50: severity = "MEDIUM"
            elif conf >= 0.25: severity = "LOW"
            else: severity = "INFO"
...
def main():
    ...
    if not scenario.exists():
        ...
        for f in sorted(Path("examples/replay").glob("*.jsonl")):
            print(f"    {f}")
        sys.exit(1)
